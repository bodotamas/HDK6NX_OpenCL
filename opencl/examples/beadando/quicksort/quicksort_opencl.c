...
int main(void)
{
    ...
    char* kernel_source = load_kernel_source("./kernels/quicksort_kernel.cl", &file_err);
    if (file_err != 0 || !kernel_source) {
        printf("Nem sikerült betölteni a kernel forrást: ./kernels/quicksort_kernel.cl\n");
    ...
    cl_kernel kernel = clCreateKernel(program, "quicksort_kernel", &err);
    if (err != CL_SUCCESS) {
    ...
    err = clEnqueueWriteBuffer(queue, buffer, CL_TRUE, 0, bytes, data, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
    ...
    err = clEnqueueReadBuffer(queue, buffer, CL_TRUE, 0, bytes, data, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
